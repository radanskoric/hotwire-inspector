...
export class PanelApp {
  constructor({
    ...
    this.themeSelectElement = themeSelectElement ?? document?.getElementById('theme-select');
  ...
  initializeTheme() {
    if (!this.themeSelectElement || !this.rootElement) {
      return;
    }

    const storedTheme = this.storage?.getItem(THEME_STORAGE_KEY);
    const theme = THEME_VALUES.has(storedTheme) ? storedTheme : 'system';

    this.themeSelectElement.value = theme;
    this.applyTheme(theme);
    this.themeSelectElement.addEventListener('change', () => {
      this.setTheme(this.themeSelectElement.value);
    });
  }
  ...
  renderNode(node) {
    ...
    row.addEventListener('mouseenter', () => {
      this.sendToInspectedTab({ type: CONTENT_HIGHLIGHT_MESSAGE_TYPE, id: node.id });
    });

    row.addEventListener('mouseleave', () => {
      this.sendToInspectedTab({ type: CONTENT_CLEAR_HIGHLIGHT_MESSAGE_TYPE });
    });

    row.addEventListener('click', async () => {
      const response = await this.sendToInspectedTab({ type: CONTENT_INSPECT_MESSAGE_TYPE, id: node.id });
