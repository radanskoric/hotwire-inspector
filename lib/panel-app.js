...
export class PanelApp {
  constructor({
    ...
    this.tabId = this.chrome.devtools.inspectedWindow.tabId;
  }
  ...
  sendToInspectedTab(message) {
    return this.chrome.tabs.sendMessage(this.tabId, message);
  }

  inspectElementBySelector(selector) {
    const escapedSelector = selector.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    const code = `inspect(document.querySelector("${escapedSelector}"))`;
    this.chrome.devtools.inspectedWindow.eval(code);
  }
